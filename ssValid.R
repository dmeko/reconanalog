ssValid<- function(y,X,ical,ival,i1) {
  ...
  # to second and first halves of data. Written because needed by resonsw4().
  #
  ...
  # at statistics of calibraton and validation (cols 1 and 2 of A). 
  ...
  PredNull<-rep(A[2,1],length(ival))# The "null" prediction, definded as the observed
  # calib-period mean for each year of the validation period; vector
  
  #--- VALIDATION
  
  mValid<-length(ival)# number of obs in validation period
  eNull <-  y[ival]-PredNull # null-prediciton  errors, computed as observed y minus 
  # prediction (vector)
