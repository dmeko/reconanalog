RecMLR1 <- function(D) {
  ...
  if (methMSR==3 | isTRUE(PCApredictors)){
    ...
    B[,1] <- ResPCA$PctVar; B[,2] <- ResPCA$CumPctVar
 ...
 # axis(1, at =(1:length(rv1U1)),labels = nmsU)
  ...
  if (methMSR==3){
    ...
    if (PCApredictors){
      strPart1 <- '\n   Number of: screened SSRs / PCs in pool / PCs in model'
      strPart2 <- ' (see Table 8 for model coefficients)'
    } else {
      strPart1 <- '\n   Number of: screened SSRs in pool / in final model'
  ...
  if (methMSR==3){
    ...
    strText <-paste(RecMethod,strPart1,
                    ...
                    '\n',strCalPd,
                    '\n   R-squared =',sprintf('%.2g',ResMLR1a$Rsquared),
  ...
  if (FigNumber<10){
    ...
    fileOut <- paste(outputDir,'Figure', as.character(FigNumber),'-AnalysisResduals1.png',sep="")
  }
  ...
  if (methMSR ==3){
    ...
    Xr <- as.matrix(U[,inmodelU3]) # matrix, long-term predictors
    L<-complete.cases(Xr)
  ...
  par(mar=c(5,5,4,2))
   acf(w2,lag.max=MaxLag,ylim=c(-1,1),type='correlation',main=Tit1)
